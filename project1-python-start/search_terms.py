...
STOP_WORDS = {
    "어떤", "어떻게", "무엇", "무엇을", "어디", "어디에", "어느", "누가", "제가", "저는",
    "하는", "하면", "해야", "하나요", "있나요", "인가요", "되나요", "해주세요",
    "알려주세요", "정리해주세요", "확인해주세요", "좋다고", "좋은", "좋을", "생기면",
    "있을까요", "안내했나요", "사용해도", "넘어가도", "대해", "대한", "것을",
    "오늘", "금일", "내일", "모레", "어제", "이번", "다음", "지난", "금주", "차주", "지금", "현재", "전에", "정리", "반드시", "점이", "듣다", "마치기", "확인해야", "생겼", "것이", "못해도", "the", "is", "a",
}
...
def split_search_terms(text):
    ...
    for word in words:
        if word in STOP_WORDS:
            continue
        # '제출해야/질문하면/이해하지'가 서로 다른 희귀 검색어가 되지 않도록
        # 명사+하다의 활용을 먼저 정리합니다. 숫자·제품명에는 적용하지 않습니다.
        if re.fullmatch(r"[가-힣]+", word):
            word = re.sub(r"(?:해야|하지|하면|하는|하기|하나요|해주세요)$", "", word)
        # 정규식으로 끝부분을 줄이는 규칙이라, 원래 단어의 일부를 조사로 오인할 수 있습니다.
        # 검색 누락이 생기면 아래 어미 목록과 최소 길이 조건부터 확인합니다.
        stem = re.sub(
            r"(?:에서는|으로는|에게는|까지는|에서도|을까요|인가요|하나요|되나요|적으로|적인|하기|하면|에게|에서|으로|까지|부터|은|는|을|를|가|이|와|과|로|에|나)$",
            "", word,
        )
        if len(stem) >= 2 or stem in {"주", "날"}:
            word = stem
...
def expand_search_terms(raw_keywords, question):
    ...
    # '슬랙/slack'처럼 같은 표기를 두 번 세어 점수가 부풀지 않도록 합니다.
    unique = {}
    for term in terms:
        unique.setdefault(compact_text(term), term)
