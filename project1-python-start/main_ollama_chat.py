...
def run_interactive_chat(all_messages, *, backend=None, keyword_backend=None):
    ...
    print("Slack LLM 질문잡담방 내용을 검색해 답변합니다.")
    print("질문을 입력하세요. 종료: 종료 / exit / quit / Ctrl+C")
    while True:
        try:
            question = input("\n질문: ").strip()
            if not question:
...
def prepare_context(all_messages, keywords, question, reference_time=None, *, trace=None):
    ...
    # 선택된 작성자와 본문에서 언급한 담당자의 이름만 조회합니다.
    author_ids = [message.get("user") for message in selected_messages]
    for message in selected_messages:
        author_ids.extend(re.findall(r"<@([A-Z0-9]+)(?:\|[^>]+)?>", message.get("text") or ""))
    if trace is not None:
...
def format_message(message, user_names=None):
    ...
    text = re.sub(
        r"<@([A-Z0-9]+)(?:\|[^>]+)?>",
        lambda match: (user_names or {}).get(match[1]) or match[0],
        message.get("text") or "",
    )
    lines = [
        f"[메시지 ts: {message['ts']} / 작성: {posted_at:%Y-%m-%d %H:%M} KST]",
        f"작성자: {author}",
        normalize_slack_links(resolve_relative_dates(text, posted_at.date())),
    ]
