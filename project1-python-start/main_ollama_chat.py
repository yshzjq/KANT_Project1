...
from evaluation_metrics import EXPERIMENT_KINDS, MeasuredClient, field, render_question, render_summary, running_model, validate_questions
...
def main(*, backend=None):
    ...
    # Luna 진입 파일은 API 연결 객체만 넘깁니다. 검색·날짜 판정·평가 분기는 함께 사용합니다.
...
def run_interactive_chat(all_messages, *, backend=None):
    ...
    while True:
        try:
            ...
            try:
                ...
                connection = {"client": backend, "model": backend.model} if backend is not None else {}
...
def answer_question(question, all_messages, client=None, *, trace=None, model=None):
    ...
    print(f"{model}의 답변을 기다리고 있습니다.", flush=True)
...
def run_question_evaluation(all_messages, *, experiment_kind="main", backend=None):
    ...
    with output_path.open("x", encoding="utf-8") as report:
        ...
        try:
            for number, item in enumerate(questions, start=1):
                ...
                try:
                    connection = {"model": model} if backend is not None else {}
